...
class JsUnpack{
  ...
  String unpack(){
    ...
    var p = RegExp(r'eval\(function\(p,a,c,k,e.+?String\.fromCharCode\(([^)]+)').firstMatch(payload);
    bool pNew = false;
    if (p?.groupCount != 0){
      pNew = RegExp(r'String\.fromCharCode\(([^)]+)').firstMatch(payload)?.group(0)?.split('+')[0] == '161';
    }
    if (pNew){
    ...
    var source = payload;
    var reg = RegExp(r"\b\w+\b",).allMatches(payload);
    int correct = 0;
  ...
  String _replaceStrings(String source){
    ...
    for(var i = 0; i < lookup.length; i++){
      ...
      if (value.contains('\\x')){
        ...
        source = source.replaceAll(variable.replaceAll("%d", i.toString()), '"$value"');
      }
    }
  ...
  String _replaceJsStrings(String source){
    ...
    for(var i = 0; i < re.groupCount; i++){
      source = source.replaceAll('\\x${re.group(i)!}', _unHexlify(re.group(i)!));
    }
    return "";
  }
...
class UnBaser{
  ...
  String _convert(int num, String base, String out)
  {
    if (num >= base.length){
      ...
      var characters = List.generate(base.codeUnits.length,(index) {
